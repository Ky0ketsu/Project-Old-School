...
public class MUSIC : MonoBehaviour
{
    ...
    // USE MUSIC.PLAYER anywhere to access the following public functions:
    // SetPlaylist()
    // Play()
    // Stop()
    // Pause()
    ...
    public void SetPlaylist(SO_Playlist desired, bool forceStart) // use forceStart=true to restart the same playlist from the beginning
    {
    ...
    {
        musicSpeed = Mathf.Clamp(desired, 0, 3f);
        ApplyMusicPitch();
    }
    ...
    void OnEnable()
    {
        EVENTS.OnTimeScaleChange += TimeScaleChangeMusicSpeed;
    }
    ...
    void SwitchPlaylist(SO_Playlist newPlaylist)
    {
        ...
        Debug.Log("🎸 SWITCH PLAYLIST TO "+newPlaylist.name);
    ...
    {
        musicPaused = false;
        while (players[current].time == 0) yield return null;
        while (players[current].time < players[current].clip.length && players[current].time > 0)
        {
            yield return null;
        }
        TrackEnded();
    ...
    void TimeScaleChangeMusicSpeed(float timeScale)
    {
        ApplyMusicPitch();
    }

    void ApplyMusicPitch()
    {
        if (currentPlaylist == null) finalMusicPitch = musicSpeed;
        else switch (currentPlaylist.timeScale)
            {
                case TimeScale.Unscaled: finalMusicPitch = musicSpeed; break;
                case TimeScale.GameSpeed: finalMusicPitch = TimeScaleManager.GameTimeScale * musicSpeed; break;
                case TimeScale.UnityTimeScale: finalMusicPitch = Time.timeScale * musicSpeed; break;
            }

        finalMusicPitch = Mathf.Clamp(finalMusicPitch, 0, 3f);

        if (currentPlaylist == null || currentMusic == null) return;

        if (musicPaused == false)
        {
            if (finalMusicPitch <= 0 && players[current].isPlaying) players[current].Pause();
            if (finalMusicPitch > 0 && players[current].isPlaying == false) players[current].UnPause();
        }

        players[0].pitch = players[1].pitch = finalMusicPitch;
    }
