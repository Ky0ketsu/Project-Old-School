...
{
    Player player;
    [SerializeField] GameObject mainCanvas,menusList,inGameList,mainMenu,pauseMenu,settingsMenu,creditsMenu,endMenu,victoryMenu; //<-- Reference to all the menus and the in-game UI
    ...
    void OnEnable()
    {
        ...
        EVENTS.OnVictory += GoToVictoryMenu;
    }
    ...
    void RemoveExtraCanvas()
    {
        foreach (Canvas myCanvas in GetComponentsInChildren<Canvas>(true))
        {
            GraphicRaycaster caster = myCanvas.GetComponent<GraphicRaycaster>();
            if (caster == null)
            {
                CanvasScaler scaler = myCanvas.transform.GetComponent<CanvasScaler>();
                if (scaler) Destroy(scaler);
                myCanvas.transform.localScale = Vector3.one;
