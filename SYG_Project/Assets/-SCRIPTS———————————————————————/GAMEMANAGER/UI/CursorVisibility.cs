...
public class CursorVisibility : MonoBehaviour
{
    ...
    [SerializeField] bool KeyboardHidesCursor = true;
    ...
    void Update()
    {
        if (MouseIsUsed() != usingMouse) // CHANGE IN MOUSE USAGE
        {
            ...
            {
                if (!KeyboardHidesCursor && ReInput.controllers.GetLastActiveControllerType()==ControllerType.Keyboard) return; // if keyboard has been touched and keyboard is not seen as a controller for this game, then stay in mouse mode
                usingMouse = false;
                SwitchControlMode();
