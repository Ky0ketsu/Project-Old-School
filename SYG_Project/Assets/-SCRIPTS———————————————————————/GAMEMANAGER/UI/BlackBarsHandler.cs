...
public class BlackBarsHandler : MonoBehaviour
{
    ...
    [SerializeField] SO_ScreenRatioList possibleRatios;
    float variance;
    ...
    int ClosestRatioIndex()
    {
        ...
        float smallestVariance = Mathf.Abs(trueScreenRatio - possibleRatios.list[0].ratio);
        for (int i = 1; i < possibleRatios.list.Length; i++)
        {
            float newVariance = Mathf.Abs(trueScreenRatio - possibleRatios.list[i].ratio);
    ...
    void SetCameraRatio()
    {
        ...
        float desiredRatio = possibleRatios.list[ClosestRatioIndex()].ratio;
        if (Application.isPlaying) Debug.Log("🖥️ New Screen size : " + Screen.width + "x" + Screen.height + "  ratio = " + Camera.main.aspect + "  variance=" + Variance + "  effective ratio " + possibleRatios.list[ClosestRatioIndex()].ratioName);
        Rect desiredRect;
        ...
        //Camera.main.rect = desiredRect;//⚠️⚠️⚠️⚠️⚠️⚠️⚠️⚠️
